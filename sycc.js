...
async function userTasks() {
    ...
    list = []
    for (let user of $.userList) {
        list.push(user.signIn())
    } await Promise.all(list)
	await wait(5)

	$.log('点赞第一次', { sp: true, console: false })  // 带分割的打印
    list = []
    for (let user of $.userList) {
        list.push(user.dzhgz())
    } await Promise.all(list)
	await wait(5)

	$.log('点赞第二次', { sp: true, console: false })  // 带分割的打印
	list = []
    for (let user of $.userList) {
        list.push(user.dzhgz())
    } await Promise.all(list)
	...
	$.log('分享', { sp: true, console: false })  // 带分割的打印
    list = []
    for (let user of $.userList) {
        list.push(user.fxpyq())
    } await Promise.all(list)
	await wait(5)


	$.log('问卷', { sp: true, console: false })  // 带分割的打印
    list = []
    for (let user of $.userList) {
        list.push(user.txwj())
    } await Promise.all(list)
	...
	$.log('浏览', { sp: true, console: false })  // 带分割的打印
    list = []
    for (let user of $.userList) {
        list.push(user.lldt())
    } await Promise.all(list)
	...
	$.log('消消卡游戏第一关', { sp: true, console: false })
    list = []
    for (let user of $.userList) {
        if (user.ckFlog) {
            list.push(user.xxkyx1())
        }
    } await Promise.all(list)
	await wait(5)

	$.log('消消卡游戏第二关', { sp: true, console: false })
    list = []
    for (let user of $.userList) {
        if (user.ckFlog) {
            list.push(user.xxkyx2())
        }
    } await Promise.all(list)
	await wait(5)

	$.log('消消卡游戏第三关', { sp: true, console: false })
    list = []
    for (let user of $.userList) {
        if (user.ckFlog) {
            list.push(user.xxkyx3())
        }
    } await Promise.all(list)
	await wait(5)
...
class UserClass {
    constructor(ck) {
        ...
        this.sy_headers = {
			...
			"Referer": "https://servicewechat.com/wx0a035430a2e3a465/79/page-frame.html"
    ...
    async dhhb() {
        ...
        if (resp.code == 200) {
            ...
            $.log(`${this.idx}: ${resp.message}`, { notify: true })
            this.ckFlog = true
    ...
    async userInfo() {
        let options = {
            fn: 'userInfo',
            method: 'get',
            url: `${this.hostname}/jeecg-boot/applet/user/userInfo`,
            headers: this.sy_headers,
        }
        // console.log(options)
        let resp = await $.request(options)
        // console.log(resp)
        if (resp.code == 200) {
            $.log(`${this.idx}: 欢迎用户: ${resp.result.weixinName} `)
			$.log(`      当前拥有积分: ${resp.result.score} `)
            this.nickname = resp.result.weixinName
            this.ckFlog = true
    ...
    async signIn() {
        let options = {
            fn: 'signIn',
            method: 'post',
            url: `${this.hostname}/jeecg-boot/applet/ccScoreRecord/signIn`,
        ...
        // console.log(resp)
        if (resp.code == 200) {
            $.log(`${this.idx}: ${this.nickname}, 签到成功`)
        } else if (resp.code == 500) {
            $.log(`${this.idx}: ${this.nickname}, ${resp.message}`)
    ...
    async xxkyx1() {
        let options = {
            fn: 'xxkyx1',
            method: 'get',
            url: `${this.hostname}/jeecg-boot/applet/user/addScore?score=3&type=12`,
            headers: this.sy_headers,
        }
        // console.log(options)
        let resp = await $.request(options)
	...
	async xxkyx2() {
        let options = {
            fn: 'xxkyx2',
            method: 'get',
            url: `${this.hostname}/jeecg-boot/applet/user/addScore?score=5&type=16`,
            headers: this.sy_headers,
        }
        // console.log(options)
        let resp = await $.request(options)
	...
	async xxkyx3() {
        let options = {
            fn: 'xxkyx3',
            method: 'get',
            url: `${this.hostname}/jeecg-boot/applet/user/addScore?score=10&type=17`,
            headers: this.sy_headers,
        }
        // console.log(options)
        let resp = await $.request(options)
    ...
    async fxpyq() {
        let options = {
            fn: 'fxpyq',
            method: 'get',
            url: `${this.hostname}/jeecg-boot/applet/user/addScore?score=1&type=14&time=0`,
            headers: this.sy_headers,
        }
        // console.log(options)
        let resp = await $.request(options)
    ...
    async dzhgz() {
        let options = {
            fn: 'fxpyq',
            method: 'get',
            url: `${this.hostname}/jeecg-boot/applet/user/addScore?score=1&type=15&time=0`,
            headers: this.sy_headers,
        }
        // console.log(options)
        let resp = await $.request(options)
    ...
    async txwj() {
        let options = {
            fn: 'txwj',
            method: 'POST',
            url: `${this.hostname}/jeecg-boot/applet/ccUserActivity/add`,
            headers: this.wj_headers,
			body: '{"latitude":23.87418646918403,"longitude":106.65275336371528,"address":"广西壮族自治区百色市右江区城东大道178号","activityId":"1567777554668478466","isOver":false}'
        }
        // console.log(options)
        let resp = await $.request(options)
        // console.log(resp)
    ...
    async lldt() {
        let options = {
            fn: 'lldt',
            method: 'get',
            url: `${this.hostname}/jeecg-boot/applet/user/addScore?score=5&type=5&time=554260`,
            headers: this.sy_headers,
        }
        // console.log(options)
        let resp = await $.request(options)
