/*
上海嘉定
cron 10 5 9,20 * * *  zsdl.js

export shjdhd=" token @ token "  
...
const envSplitor = ['&', '@']     //支持多种分割，但要保证变量里不存在这个字符
const ckNames = ['shjdhd']                //支持多变量
//====================================================================================================
...
async tykl() {
    let options = {
        fn: 'tykl',
        method: 'plm',
        url: `${this.hostname}/media-basic-port/api/app/personal/score/info`,
 ...
 if (resp.code == 0) {
    ...
    $.log(`${this.idx}: 积分： ${resp.data.totalScore}`)
    let tasks = resp.data.jobs
...
/*
            ...
            case 1:
                if (element.status < 1) {
                    $.log(`${this.idx}: 未登录，去执行登录`)
                    //await this.signIn()
                    //await this.doLottery()
                } else if (element.allProgress == 1) {
                    $.log(`${this.idx}:  今天已经登录过了鸭，明天再来吧！`)
                }
                break
            
            case 2:
                if (element.status < 1) {
    $.log(`${this.idx}:${element.id} ${element.status}  ${element.totalProgress}   ${element.progress}`)
                    let num = element.totalProgress - element.allProgress
                    for (let index = 0; index <= num; index++) {
                        await run()
                        await this.fqz()
                    }
                } else if (element.allProgress == 2) {
                    $.log(`${this.idx}:  今天已经发圈子了，明天再来吧!`)
                }
                break    
/*
            case 3:
    ...
    $.log(`${this.idx}:${element.id} ${element.status}  ${element.totalProgress}   ${element.progress}`)
                ...
                } else if (element.allProgress == 10) {
                    $.log(`${this.idx}:  今天已经发表评论了，明天再来吧!`)
                }
                break    
...
async sign() {
    let options = {
        fn: 'sign',
        method: 'post',
        url: `${this.hostname}/media-basic-port/api/app/personal/score/sign`,
        headers: this.qd_headers,
        body: ''
    }
    // console.log(options)
 ...
 if (resp.code == 0) {
    ...
    async ydwz() {
        let options = {
            fn: 'ydwz',
            method: 'post',
            url: `${this.hostname}/media-basic-port/api/app/points/read/add`,
...
async stjf() {
    let options = {
        fn: 'stjf',
        method: 'post',
        url: `${this.hostname}/media-basic-port/api/app/points/video/add`,
        headers: this.wu_headers,
    }
    // console.log(options)
    let resp = await $.request(options)
...
async fbpl() {
    let options = {
        fn: 'fbpl',
        method: 'post',
        url: `${this.hostname}/media-basic-port/api/app/common/comment/add`,
        headers: this.wu_headers,
        body: {"content":"...","displayResources":[],"targetId":"af12b06229da4aebb22b4f4112d84ffa","targetType":"content"}
    // console.log(options)
    let resp = await $.request(options)
    // console.log(resp)
 if (resp.code == 0) {
...
async fqz() {
    let options = {
        fn: 'fqz',
        method: 'post',
        url: `${this.hostname}/media-basic-port/api/app/moment/status/add`,
        headers: this.fqz_headers,
        form: {
            "content": "  ",
            "resources": [],
            "tribe": {
              "id": "76f65aff44e64a8391822e79540223849f91",
              "requestType": "1",
              "siteId": "310114"
            },
            "requestType": "1",
...
async zffx() {
    let options = {
        fn: 'zffx',
        method: 'post',
        url: `${this.hostname}/media-basic-port/api/app/points/share/add`,
        headers: this.wu_headers,
    }
    // console.log(options)
    let resp = await $.request(options)
