/*
每日阅读            cron 10 5 9,20 * * *  zsdl.js

export mryd=" token @ token "  
...
async function userTasks() {
    ...
    $.log('每日阅读签到', { sp: true, console: false })  // 带分割的打印
    list = []
    ...
    $.log('每日阅读开始', { sp: true, console: false })  // 带分割的打印
    list = []
    for (let user of $.userList) {
        list.push(user.cs())
    } await Promise.all(list)
...
class UserClass {
    constructor(ck) {
		...
		this.mr_headers = {
            ...
            Referer: 'http://jijianfu.com/index',
    ...
    async mrqd() {
        let options = {
            fn: 'mrqd',
            method: 'GET',
            url: `${this.hostname}/api/userSign/sign`,
            headers: this.mr_headers,
        }
        // console.log(options)
    ...
    async cs() {
        ...
        async mr1() {
            let options = {
                fn: 'mr1',
                method: 'GET',
                url: `${this.hostname}/api/task/tasks`,
                headers: this.mr_headers,
            }
            // console.log(options)
            let resp = await $.request(options)
            // console.log(resp)
         if (resp.code == 200) {
            $.log(`${this.idx}:\n今日阅读数${resp.data.reward}\n\n积分数${resp.data.reward}\n`)
            for (let i = 0; i < 3; i++) {
             this.id = resp.data.list[i].uid
             await wait(1)
             await this.mr2()
             await run()
             await this.mr3()
    ...
    async mr2() {
        let options = {
            fn: 'mr2',
            method: 'GET',
            url: `${this.hostname}/api/task/readTasks?uid=${this.id}`,
            headers: this.mr_headers,
        }
        // console.log(options)
        let resp = await $.request(options)
...
async mr3() {
    let options = {
        fn: 'mr3',
        method: 'GET',
        url: `${this.hostname}/api/task/applyTask?uid=${this.id}`,
        headers: this.mr_headers,
    }
    // console.log(options)
    let resp = await $.request(options)
